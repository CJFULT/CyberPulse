// frontend/src/components/CategoryCard.tsx
import React from 'react';
import { Shield, Bug, Server, Users, BarChart, Eye, ExternalLink, TrendingUp } from 'lucide-react';
import { Category } from '../types';

export const categoryIcons: { [key: string]: React.ElementType } = {
  "Ransomware": Shield,
  "Data Breaches & Leaks": Bug,
  "Cloud Security Incidents": Server,
  // ... add all your other category-to-icon mappings here
  "Default": BarChart, 
};

interface CategoryCardProps {
  category: Category;
  onClick: () => void;
}

export const CategoryCard: React.FC<CategoryCardProps> = ({ category, onClick }) => {
  // --- SAFEGUARD 1: Handle undefined category prop ---
  // If the category object doesn't exist yet, render nothing to prevent a crash.
  if (!category) {
    return null;
  }

  // --- SAFEGUARD 2: Safely access properties with defaults ---
  // This prevents crashes if a specific field is missing from the data.
  const gradient = category.gradient || 'from-gray-500 to-gray-600';
  const totalViews = category.totalViews ?? 0;
  const articles = category.articles ?? [];
  const IconComponent = categoryIcons[category.name] || categoryIcons.Default;

  return (
    <div
      onClick={onClick}
      className="group relative overflow-hidden rounded-xl bg-gradient-to-br from-gray-900/90 to-gray-800/90 border border-gray-700/50 hover:border-cyan-500/50 transition-all duration-300 cursor-pointer hover:scale-105 hover:shadow-2xl hover:shadow-cyan-500/20"
    >
      <div className={`absolute inset-0 bg-gradient-to-br ${gradient} opacity-0 group-hover:opacity-10 transition-opacity duration-300`} />
      
      <div className="relative p-6">
        <div className="flex items-center justify-between mb-4">
          <div className={`w-12 h-12 rounded-lg bg-gradient-to-br ${gradient} flex items-center justify-center`}>
            {/* Use the dynamically selected icon component */}
            <IconComponent className="h-6 w-6 text-white" />
          </div>
          <div className="flex items-center space-x-2 text-gray-400">
            <Eye className="h-4 w-4" />
            {/* Use the safe totalViews variable */}
            <span className="text-sm">{totalViews.toLocaleString()}</span>
          </div>
        </div>

        <h3 className="text-xl font-bold text-white mb-2 group-hover:text-cyan-400 transition-colors">
          {category.name}
        </h3>

        <div className="bg-gray-800/50 rounded-lg p-3 mb-4">
          <p className="text-gray-300 text-sm leading-relaxed">
            {category.summary}
          </p>
        </div>

        <div className="flex items-center justify-between">
          <span className="text-sm text-gray-400">
            {articles.length} articles
          </span>
          <ExternalLink className="h-4 w-4 text-gray-400 group-hover:text-cyan-400 transition-colors" />
        </div>
      </div>
    </div>
  );
};